...
\begin{tikzpicture}[node distance=1.2cm, auto]
    ...
    \node[server, below left=1.0cm and 3.5cm of ingress, align=center] (smartshell) {smart-shell \\ \small(raspberry-002)};
...
\begin{tikzpicture}[node distance=1.2cm, auto]
    ...
    \node[server, below=0.5cm of internet, align=center] (vps) {FRPS (Servidor) \\ \small161.132.4.98};
    \node[server, below=1.0cm of vps, align=center] (frpc) {FRPC (Cliente) \\ \small(En Clúster K8s)};
    \node[component, below=0.5cm of frpc, align=center] (ingress_ip) {IP de Ingress \\ \small192.168.100.240};
...
\section{Componentes de Balanceo de Carga}

\begin{figure}[H]
\centering
\begin{tikzpicture}[node distance=1.2cm, auto]
    % Nodos
    \node[component] (lb) {Balanceo de Carga};
    \node[server, below left=1.2cm and -2cm of lb] (cp_lb) {Plano de Control};
    \node[server, below right=1.2cm and -2cm of lb] (svc_lb) {Exposición de Servicios};

    \node[component, below=of cp_lb] (keepalived) {Keepalived};
    \node[component, below=of keepalived] (haproxy) {HAProxy};

    \node[component, below=of svc_lb] (metallb) {MetalLB};
    \node[component, below=of metallb] (nginx) {Nginx Ingress};
    \node[component, below=of nginx] (frp) {FRP};

    % Conexiones
    \draw[flow] (lb) -- (cp_lb);
    \draw[flow] (lb) -- (svc_lb);

    \draw[flow] (cp_lb) -- (keepalived);
    \draw[flow] (keepalived) -- (haproxy);

    \draw[flow] (svc_lb) -- (metallb);
    \draw[flow] (metallb) -- (nginx);
    \draw[flow] (nginx) -- (frp);
\end{tikzpicture}
\caption{Componentes de Balanceo de Carga.}
\label{fig:lb_components_diagram}
\end{figure}
